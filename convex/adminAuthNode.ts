...
export const signInWithPassword = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    const failures = attempts.filter((attempt: { successful: boolean; createdAt: number }) => !attempt.successful).map((attempt: { successful: boolean; createdAt: number }) => attempt.createdAt)
